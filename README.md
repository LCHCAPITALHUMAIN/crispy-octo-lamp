...
1. Rename the `src/resize_image` package.
1. Globally replace (*case-sensitive*) instances of `resize-image`, `resize_image`, and `resize_image` with your project and package name (including later in this list).
...
1. Configure a Cloud Build trigger in your GCP project to deploy from `cloudbuild.yaml` on a new tag that matches the regex `^(v(\d+))$`, with a substituion variable `_resize_image_API_DOMAIN` pointing to the (sub)-domain you want to host the API on. Make sure the cloudbuild service account has the required permissions (Cloud Build settings --> enable Cloud Run, Service Accounts, and Cloud Build) and is [registered as an owner of your custom domain](https://stackoverflow.com/a/70510793) if required.
...
1. Create and test your API. Your app under `resize_image.api:app` will be hosted on CloudRun when it's deployed.
...
Note: if Poetry is managing a virtual environment for you, you may need to use `poetry run poe local-server` instead of `poe`

- `poe autoformat` - Autoformat code
- `poe lint` - Linting
- `poe test` - Run Tests
- `poetry run poe local-server` - Run your API locally
